...
- **多客户端资源**：支持官服、TapTap、Bilibili、华为、应用宝、OPPO、VIVO 等资源配置。
- **任务配置**：任务启用开关、搜索筛选、任务选项和参数保存。
- **后台运行**：通过独立虚拟显示器运行阴阳师，手机前台可以继续使用；支持实时预览、全屏手动操作、返回前台和关闭后台。
...
4. 前台运行时直接点击任务详情中的“开始任务”。
5. 后台运行时打开“后台”页启动游戏，然后点击“开始任务”；它会使用任务页最后保存的任务和参数。手动操作时点击预览右上角的按钮即可进入全屏，点击悬浮叉号返回后台预览。
6. 定时运行时打开“定时”页，选择周几和任意多个时间点，保存设置。定时页不重复配置任务，任务范围始终继承“任务”页的开关。
...
```powershell
.\gradlew.bat :testDebugUnitTest :app:lintDebug :app:connectedDebugAndroidTest
```

后台虚拟显示器测试需要一个专用、可执行 `adb root` 的测试模拟器：

```powershell
python scripts/test-background.py --serial 127.0.0.1:16416
```
