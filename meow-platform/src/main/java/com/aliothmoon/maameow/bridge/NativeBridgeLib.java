...
public class NativeBridgeLib {
    ...
    static {
        try {
            // MaaYYs adaptation: app_process/Shizuku may not have the APK native
            // directory on java.library.path. The host supplies its absolute path.
            String explicitPath = System.getProperty("maayys.bridge.library");
            if (explicitPath != null) System.load(explicitPath);
            else System.loadLibrary("bridge");
            LOADED = true;
        } catch (Throwable e) {
            LOADED = false;
